...
import { Box, CircularProgress, Divider, Grid, Pagination, Typography } from "@mui/material";
...
function RoomTypeList() {
  ...
  return (
    <Box width={"100%"} display={"flex"} flexDirection={"column"} alignItems={"center"} mt={"40px"}>
      <Box
        width={contentWidth}
        display="flex"
        ...
        py={7}
      >
        <Box ref={boxRef} display="flex" flexDirection="column" alignItems={"center"} gap={2} mb={7}>
            ...
            <Grid container spacing={2}>
              {dataHouse.length > 0 &&
                dataHouse?.map((room, index) => (
                  <Grid item xs={6}  className={ANIMATION_REVEAL_ELEMENT.LEFT_TO_RIGHT.default} key={room.id} sx={{pl: index % 2 !== 0 ?  "36px!important" : "16px!important", mb: "20px!important"}}>
